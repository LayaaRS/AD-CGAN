...
class Generator(nn.Module):
    def __init__(self, latent_size):
        ...
        self.deconv.append(
            nn.Sequential(
                SpectralNorm(nn.ConvTranspose2d(self.latent_dim,
                             1024, 5, stride=2, bias=True, padding=1)),
                nn.BatchNorm2d(1024),
                nn.ReLU(),
            )
        )
...
class EncoderResNet18(nn.Module):
    ...
    def __init__(self, latent_size, dropout=0.3, noise=False):
        ...
        self.__in_features = nn.Linear(512, self.latent_size)
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        x = self.__in_features(x)
        x = x.view((x.shape[0], self.latent_size, 1, 1))
        # print(x.shape)
        return x

    # def output_num(self):
    #     return print(self.__in_features.shape)
...
class EncoderResNet18_28(nn.Module):
    """Encoder module of the CIFAR10 and SVHN datasets.
    It uses spectral normalization."""

    def __init__(self, latent_size, dropout=0.3, noise=False):
        super(EncoderResNet18_28, self).__init__()
        self.dropout = dropout
        self.latent_size = latent_size
        self.leaky_value = 0.2
        self.conv = nn.ModuleList()
