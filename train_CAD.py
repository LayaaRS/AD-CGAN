...
from modules.AD_CGAN import losses
from modules.AD_CGAN import models, models_lg
...
parser.add_argument("-elr", "--enc_learning_rate", default=1e-4,
                    type=float, help="encoder learning rate")
parser.add_argument("-bs", "--batch_size", default=32,
                    type=int, help="batch size")
parser.add_argument("-ep", "--num_epochs", default=20,
                    type=int, help="number of epochs")
parser.add_argument("-wep", "--warmup_epochs", default=0,
                    type=int, help="number of warm up epochs for autoencoder")
parser.add_argument("-z", "--latent_size", default=100,
                    type=int, help="latent size")
parser.add_argument(
    "-dz", "--distribution", default="Gaussian", help="choose the distribution you want to z selected from"
)
parser.add_argument("-d", "--dropout", default=0.2,
                    type=float, help="daropout")
parser.add_argument("-ls", "--GAN_loss", default="CGAN",
                    help="SGAN/RSGAN/WGAN/CGAN")
parser.add_argument("-sc", "--scale", default=0.3,
                    type=float, help="InfoMax scale")
...
if args.dataset == "CIFAR10":
    ...
    enc = models.EncoderResNet18(args.latent_size, args.dropout)
    gen = models_lg.Generator(args.latent_size)
    dis = models_lg.Discriminator(args.latent_size, args.dropout)
    dis_z = models.Discriminator_zz(args.latent_size, args.dropout)
elif args.dataset == "MNIST" or args.dataset == "FashionMNIST":
    # enc = models.Encoder_MNIST(args.latent_size, args.dropout)
    enc = models.EncoderResNet18_28(args.latent_size, args.dropout)
    gen = models_lg.Generator_MNIST(args.latent_size)
    dis = models_lg.Discriminator_MNIST(args.latent_size, args.dropout)
    dis_z = models.Discriminator_zz_MNIST(args.latent_size, args.dropout)
elif args.dataset == "CatsVsDogs":
    enc = models.Encoder_CD(args.latent_size, args.dropout)
    gen = models_lg.Generator_CD(args.latent_size)
    dis = models_lg.Discriminator_CD(args.latent_size, args.dropout)
...
optimizer_d = optim.AdamW(dis.parameters(), lr=args.learning_rate, betas=(
    beta1, beta2), weight_decay=0.01)
optimizer_g = optim.AdamW(gen.parameters(), lr=args.learning_rate, betas=(
    beta1, beta2), weight_decay=0.05)
optimizer_e = optim.AdamW(enc.parameters(), lr=args.enc_learning_rate, betas=(
    beta1, beta2), weight_decay=0.05)
potimizer_z = optim.Adam(dis_z.parameters(), lr=args.learning_rate, betas=(
    beta1, beta2), weight_decay=0.05)
...
for epoch in range(1, args.num_epochs + 1):
    ...
    for iteraion, batch in enumerate(trainloader):
        ...
        if args.GAN_loss == "RSGAN" or args.GAN_loss == "CGAN" or args.GAN_loss == "WGAN":
            valid_label = Variable(torch.ones(
                (x.size(0)), dtype=torch.float), requires_grad=False).to(device)
        ...
        if epoch <= args.warmup_epochs:
            ...
            encoded = enc(x)
            # gen(encoded)/2+0.5 or (gen(encoded) + 1) /2
            ...
            for i in range(2):
                # gen(z_random)/2+0.5, scale from [-1,1] to [0,1]
                ...
                # StandardGAN loss
                    ...
                    loss_real = dis_criterion(
                        dis_real.reshape(-1, 1), valid_label)
                    loss_fake = dis_criterion(
                        dis_fake.reshape(-1, 1), fake_label)
                    loss_dis = loss_real + loss_fake
                # Contrastive Learning loss
                elif args.GAN_loss == "CGAN":
                    errD = dis_criterion(dis_real - dis_fake, valid_label)
                    errD_IM = losses.compute_infomax_loss(
                        feat_lc_r, feat_gl_r, args.scale)
                    loss_dis = errD + errD_IM
                elif args.GAN_loss == "WGAN":
                    loss_dis = (
                        dis_fake.mean() - dis_real.mean() +
                        losses.gradient_penalty(dis, x, x_fake, args.gp_weight)
                    )
            ...
            if args.GAN_loss == "RSGAN":
                ...
                errG_IM = losses.compute_infomax_loss(
                    feat_lc_f, feat_gl_f, args.scale)
            ...
            z_real_dis = dis_criterion(z_logit_real, Variable(
                torch.ones_like(z_logit_real), requires_grad=True))
            z_fake_dis = dis_criterion(z_logit_fake, Variable(
    ...
    if epoch <= args.warmup_epochs:
        print("epoch {}, enc loss: {:.2f},".format(
            epoch, loss_aen.item()), flush=True)
    else:
        print(
            "epoch {},dis loss: {:.2f}, gen loss: {:.2f}, enc loss: {:.2f},".format(
        ...
        img = torch.cat(
            (x[:8].cpu().detach(), x_fake.cpu().detach(), x_hat.cpu().detach()), 0)
...
for step, (images, labels) in enumerate(testloader_normal, 0):
    ...
    loss_test = ((1 - lam) * resloss) + (lam * discloss) + \
        (beta * latentloss) + ((1 - beta) * feature_loss)
...
for step, (images, labels) in enumerate(testloader_anomalous, 0):
    ...
    loss_test = ((1 - lam) * resloss) + (lam * discloss) + \
        (beta * latentloss) + ((1 - beta) * feature_loss)
    ## for ablation studies without beta
    # loss_test = ((1 - lam) * resloss) + (lam * discloss)
